...
export async function runBridge(
  ...
  if (balance >= config.bridgeThreshold) {
    ...
    await safeLog("bridge", log, walletClient, publicClient, config.workflowLogAddress, "bridge", msg, "skip")
    return
  }

  const msg = `ETH balance: ${formatUnits(balance, 6)} below threshold ${formatUnits(config.bridgeThreshold, 6)} → bridge request`
  console.log(`[bridge] ${msg}`)
  await safeLog("bridge", log, walletClient, publicClient, config.workflowLogAddress, "bridge", msg, "success")
}
